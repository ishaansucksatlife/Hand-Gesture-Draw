...
Copyright (C) 2025 Ishaan Banerjee
...
Source: https://github.com/ishaansucksatlife/Hand-Gesture-Draw

PROPRIETARY NOTICE: This contains original gesture recognition algorithms
and real-time computer vision implementation by Ishaan Banerjee.
Unauthorized claiming of authorship is prohibited.
"""
...
class HandDrawing:
	def __init__(self):
		try:self.mp_hands=mp.solutions.hands;self.mp_draw=mp.solutions.drawing_utils;self.hands=self.mp_hands.Hands(static_image_mode=False,max_num_hands=2,min_detection_confidence=.8,min_tracking_confidence=.8,model_complexity=1)
		except Exception as e:print(f"Error initializing MediaPipe: {e}");raise
		self.colors=[(0,255,0),(255,0,0),(0,0,255),(255,255,0),(255,0,255),(0,255,255),(255,255,255),(128,128,128),(0,165,255),(128,0,128),(128,128,0),(0,128,128),(203,192,255),(42,42,165),(0,0,0),(130,0,75)];self.color_names=['GREEN','BLUE','RED','CYAN','MAGENTA','YELLOW','WHITE','GRAY','ORANGE','PURPLE','TEAL','OLIVE','PINK','BROWN','BLACK','INDIGO'];self.current_color_index=0;self.color=self.colors[self.current_color_index];self.brush_sizes=[3,6,9,12,15,20];self.current_brush_size=2;self.brush_thickness=self.brush_sizes[self.current_brush_size];self.eraser_sizes=[15,25,35,50];self.current_eraser_size=1;self.eraser_thickness=self.eraser_sizes[self.current_eraser_size];self.drawing_tools=['brush','spray','fill'];self.current_tool=0;self.spray_density=20;self.spray_radius=15;self.canvas=None;self.history=deque(maxlen=20);self.redo_stack=deque(maxlen=20);self.left_hand={'drawing':False,'tool_active':False,'prev_point':None,'current_point':None,'points_buffer':deque(maxlen=5),'active':False,'tool_change_triggered':False,'brush_change_triggered':False,'brush_change_start_time':0,'brush_change_held':False,'stable_points':deque(maxlen=8),'last_active_time':0,'velocity':(0,0),'acceleration':(0,0),'gesture_confidence':0,'smooth_position':None,'last_gesture':'none','gesture_start_time':0,'gesture_cooldown':0};self.right_hand={'drawing':False,'erasing':False,'prev_point':None,'current_point':None,'points_buffer':deque(maxlen=5),'active':False,'color_change_triggered':False,'stable_points':deque(maxlen=8),'last_active_time':0,'velocity':(0,0),'acceleration':(0,0),'gesture_confidence':0,'smooth_position':None,'last_gesture':'none','gesture_start_time':0,'gesture_cooldown':0};self.gesture_cooldowns={'color_change':0,'brush_change':0,'tool_change':0,'drawing':0,'erasing':0};self.frame_count=0;self.fps=0;self.fps_time=time.time();self.frame_times=deque(maxlen=60);self.performance_stats={'min_fps':float('inf'),'max_fps':0,'avg_fps':0};self.hand_timeout_threshold=3.
		try:
			self.cap=cv2.VideoCapture(0)
			if not self.cap.isOpened():raise RuntimeError('Could not open camera.')
			self.cap.set(cv2.CAP_PROP_FRAME_WIDTH,1280);self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT,720);self.cap.set(cv2.CAP_PROP_FPS,60)
		except Exception as e:print(f"Error initializing camera: {e}");raise
		try:os.makedirs('saved_drawings',exist_ok=True)
		except Exception as e:print(f"Error creating saves directory: {e}")
	...
	def calculate_hand_metrics(self,hand_landmarks,hand):
		try:
			...
			hand['stable_points'].append(wrist);thumb_tip=self.get_landmark_coords(hand_landmarks,self.mp_hands.HandLandmark.THUMB_TIP);pinky_tip=self.get_landmark_coords(hand_landmarks,self.mp_hands.HandLandmark.PINKY_TIP)
			...
			if hand['smooth_position']and len(hand['stable_points'])>=2:
				current_pos=wrist;last_pos=hand['smooth_position'];dt=1./max(self.fps,30);vx=(current_pos[0]-last_pos[0])/dt;vy=(current_pos[1]-last_pos[1])/dt;hand['velocity']=vx,vy
				if hand['velocity'][0]!=0 or hand['velocity'][1]!=0:ax=vx/dt;ay=vy/dt;hand['acceleration']=ax,ay
			hand['smooth_position']=wrist;return hand_size,stability,self.calculate_gesture_confidence(hand)
	...
	def use_fill_tool(self,position):
		if position is None:return
		int_position=int(position[0]),int(position[1]);temp_canvas=self.canvas.copy();h,w=temp_canvas.shape[:2];mask=np.zeros((h+2,w+2),np.uint8);fill_color=self.color;tolerance=20;seed_point=int_position[0],int_position[1]
		if 0<=seed_point[0]<w and 0<=seed_point[1]<h:
			...
			lower_bound=np.array([max(0,c-tolerance)for c in seed_color]);upper_bound=np.array([min(255,c+tolerance)for c in seed_color]);flags=65284|cv2.FLOODFILL_FIXED_RANGE;cv2.floodFill(temp_canvas,mask,seed_point,fill_color,loDiff=tuple(lower_bound-seed_color),upDiff=tuple(upper_bound-seed_color),flags=flags);self.canvas=temp_canvas;self.save_state()
	def save_state(self):
		try:
	...
	def process_left_hand_tools(self,hand_landmarks):
		...
		if is_brush_change_gesture and brush_change_conf>.6:
			...
			if thumb_brush_change and ring_tip:
				progress=min(hold_duration/1.,1.);center_x=int((thumb_brush_change[0]+ring_tip[0])/2);center_y=int((thumb_brush_change[1]+ring_tip[1])/2);radius=25;confidence_color=0,int(255*brush_change_conf),255;cv2.circle(self.frame,(center_x,center_y),radius,confidence_color,2);angle=int(360*progress);cv2.ellipse(self.frame,(center_x,center_y),(radius,radius),0,0,angle,confidence_color,4)
				if hold_duration<1.:remaining=1.-hold_duration;cv2.putText(self.frame,f"{remaining:.1f}s",(center_x-15,center_y+5),cv2.FONT_HERSHEY_SIMPLEX,.6,confidence_color,2)
				else:cv2.putText(self.frame,'CHANGE!',(center_x-30,center_y+5),cv2.FONT_HERSHEY_SIMPLEX,.6,(0,255,0),2)
	...
	def process_right_hand_gestures(self,hand_landmarks):
		...
		if is_draw_pinch and not is_erase_pinch and draw_confidence>.5 and('drawing',draw_confidence)in active_gestures:
			...
			if pinch_center:
				...
				if hand['prev_point']is not None:cv2.line(self.canvas,hand['prev_point'],int_position,(0,0,0),self.eraser_thickness,lineType=cv2.LINE_AA)
	...
	def draw_interface(self,frame,left_gesture,right_gesture,show_interface=True):
		if not show_interface:return
		panel_width=550;panel_height=300;cv2.rectangle(frame,(10,10),(panel_width,panel_height),(40,40,60),-1);cv2.rectangle(frame,(10,10),(panel_width,panel_height),(200,200,200),2);cv2.putText(frame,'Advanced Hand Drawing',(20,35),cv2.FONT_HERSHEY_SIMPLEX,.7,(255,255,255),2);fps_color=(0,255,0)if self.fps>30 else(0,255,255)if self.fps>15 else(0,100,255);cv2.putText(frame,f"FPS: {self.fps:.1f}",(panel_width-100,35),cv2.FONT_HERSHEY_SIMPLEX,.5,fps_color,1);cv2.putText(frame,f"Avg: {self.performance_stats['avg_fps']:.1f}",(panel_width-100,55),cv2.FONT_HERSHEY_SIMPLEX,.4,(200,200,200),1);current_tool=self.drawing_tools[self.current_tool].upper();color_name=self.color_names[self.current_color_index];cv2.putText(frame,f"Tool: {current_tool}",(20,65),cv2.FONT_HERSHEY_SIMPLEX,.6,(255,255,255),2);cv2.putText(frame,f"Color: {color_name}",(20,95),cv2.FONT_HERSHEY_SIMPLEX,.6,self.color,2);cv2.circle(frame,(500,90),18,self.color,-1);cv2.circle(frame,(500,90),18,(255,255,255),2);cv2.putText(frame,f"Brush: {self.brush_thickness}px",(20,125),cv2.FONT_HERSHEY_SIMPLEX,.5,(200,200,200),1);cv2.putText(frame,f"Eraser: {self.eraser_thickness}px",(150,125),cv2.FONT_HERSHEY_SIMPLEX,.5,(200,200,200),1);left_confidence=self.left_hand.get('gesture_confidence',0);right_confidence=self.right_hand.get('gesture_confidence',0);left_timeout=time.time()-self.left_hand['last_active_time']>self.hand_timeout_threshold;right_timeout=time.time()-self.right_hand['last_active_time']>self.hand_timeout_threshold;left_status=f"Left: {left_gesture.upper()}"if left_gesture!='none'else'Left: Ready';right_status=f"Right: {right_gesture.upper()}"if right_gesture!='none'else'Right: Ready'
		if left_timeout or not self.left_hand['active']:left_color=100,100,100;left_status='Left: Timeout'
		else:left_color=(200,255,200)if left_confidence>.5 else(255,255,200)if left_confidence>.3 else(200,200,255)
		if right_timeout or not self.right_hand['active']:right_color=100,100,100;right_status='Right: Timeout'
		else:right_color=(200,255,200)if right_confidence>.5 else(255,255,200)if right_confidence>.3 else(255,200,200)
		cv2.putText(frame,left_status,(20,155),cv2.FONT_HERSHEY_SIMPLEX,.5,left_color,1);cv2.putText(frame,right_status,(20,175),cv2.FONT_HERSHEY_SIMPLEX,.5,right_color,1)
		if self.left_hand['active']and not left_timeout:cv2.putText(frame,f"L Conf: {left_confidence:.2f}",(250,155),cv2.FONT_HERSHEY_SIMPLEX,.4,left_color,1)
		if self.right_hand['active']and not right_timeout:cv2.putText(frame,f"R Conf: {right_confidence:.2f}",(250,175),cv2.FONT_HERSHEY_SIMPLEX,.4,right_color,1)
		features=['Advanced Tracking: ON','Conflict Prevention: ON','Performance Monitor: ON','Hand Timeout: ON',f"Tools: {', '.join(self.drawing_tools)}"]
		for(i,feature)in enumerate(features):cv2.putText(frame,feature,(20,200+i*18),cv2.FONT_HERSHEY_SIMPLEX,.4,(150,255,150),1)
		instructions=['LEFT: Index=Brush Middle=Spray Ring=BrushSize Pinky=Fill','RIGHT: Index=Draw Middle=Erase Pinky=Color','Keys: Z=Undo Y=Redo C=Clear S=Save T=Tool Q=Quit']
		for(i,instruction)in enumerate(instructions):cv2.putText(frame,instruction,(20,275+i*18),cv2.FONT_HERSHEY_SIMPLEX,.35,(200,200,200),1)
	def draw_advanced_gesture_feedback(self,frame,hand_landmarks,gesture,position,hand_type):
		...
		if wrist:
			wrist_int=int(wrist[0]),int(wrist[1]);label='LEFT HAND (TOOLS)'if hand_type=='left'else'RIGHT HAND (DRAWING)';label_color=(255,150,150)if hand_type=='left'else(150,150,255);cv2.putText(frame,label,(wrist_int[0]-60,wrist_int[1]-20),cv2.FONT_HERSHEY_SIMPLEX,.7,label_color,2)
			if hand_type=='left':tool_info=f"Tool: {self.drawing_tools[self.current_tool].upper()}";cv2.putText(frame,tool_info,(wrist_int[0]-30,wrist_int[1]-45),cv2.FONT_HERSHEY_SIMPLEX,.6,(255,255,0),2)
			else:stability_info=f"Conf: {confidence:.2f}";cv2.putText(frame,stability_info,(wrist_int[0]-25,wrist_int[1]-45),cv2.FONT_HERSHEY_SIMPLEX,.5,confidence_color,2)
			vx,vy=hand['velocity'];speed=math.sqrt(vx**2+vy**2)
			if speed>10:arrow_end=int(wrist[0]+vx*.1),int(wrist[1]+vy*.1);cv2.arrowedLine(frame,wrist_int,arrow_end,(0,255,255),2)
		if hand_type=='left':
			if gesture.startswith('tool_'):
				tool_name=gesture.split('_')[1];thumb_tip=self.get_landmark_coords(hand_landmarks,self.mp_hands.HandLandmark.THUMB_TIP)
				if tool_name=='brush':finger_tip=self.get_landmark_coords(hand_landmarks,self.mp_hands.HandLandmark.INDEX_FINGER_TIP);gesture_color=0,255,0
				elif tool_name=='spray':finger_tip=self.get_landmark_coords(hand_landmarks,self.mp_hands.HandLandmark.MIDDLE_FINGER_TIP);gesture_color=0,255,255
				elif tool_name=='fill':finger_tip=self.get_landmark_coords(hand_landmarks,self.mp_hands.HandLandmark.PINKY_TIP);gesture_color=255,0,255
				if thumb_tip and finger_tip:thumb_int=int(thumb_tip[0]),int(thumb_tip[1]);finger_int=int(finger_tip[0]),int(finger_tip[1]);cv2.circle(frame,thumb_int,12,gesture_color,-1);cv2.circle(frame,finger_int,12,gesture_color,-1);cv2.line(frame,thumb_int,finger_int,gesture_color,3);cv2.putText(frame,f"TOOL: {tool_name.upper()}",(thumb_int[0]+20,thumb_int[1]),cv2.FONT_HERSHEY_SIMPLEX,.7,gesture_color,2)
			elif gesture=='brush_change':
				thumb_tip=self.get_landmark_coords(hand_landmarks,self.mp_hands.HandLandmark.THUMB_TIP);ring_tip=self.get_landmark_coords(hand_landmarks,self.mp_hands.HandLandmark.RING_FINGER_TIP)
				if thumb_tip and ring_tip:thumb_int=int(thumb_tip[0]),int(thumb_tip[1]);ring_int=int(ring_tip[0]),int(ring_tip[1]);cv2.circle(frame,thumb_int,12,(0,200,255),-1);cv2.circle(frame,ring_int,12,(0,200,255),-1);cv2.line(frame,thumb_int,ring_int,(0,200,255),3);cv2.putText(frame,'BRUSH SIZE',(thumb_int[0]+20,thumb_int[1]),cv2.FONT_HERSHEY_SIMPLEX,.7,(0,200,255),2)
		elif hand_type=='right':
			if gesture=='drawing':
				thumb_tip=self.get_landmark_coords(hand_landmarks,self.mp_hands.HandLandmark.THUMB_TIP);index_tip=self.get_landmark_coords(hand_landmarks,self.mp_hands.HandLandmark.INDEX_FINGER_TIP)
				if thumb_tip and index_tip:thumb_int=int(thumb_tip[0]),int(thumb_tip[1]);index_int=int(index_tip[0]),int(index_tip[1]);cv2.circle(frame,thumb_int,10,confidence_color,-1);cv2.circle(frame,index_int,10,confidence_color,-1);cv2.line(frame,thumb_int,index_int,confidence_color,3)
				if position:
					pos_int=int(position[0]),int(position[1])
					if self.current_tool==0:cv2.circle(frame,pos_int,self.brush_thickness//2,self.color,2);cv2.circle(frame,pos_int,4,(255,255,255),-1)
					elif self.current_tool==1:cv2.circle(frame,pos_int,self.spray_radius,(255,255,255),2);cv2.circle(frame,pos_int,3,self.color,-1)
					elif self.current_tool==2:cv2.circle(frame,pos_int,8,(255,255,255),2);cv2.circle(frame,pos_int,6,(255,0,255),-1)
				if wrist:cv2.putText(frame,'ACTIVE DRAWING',(wrist_int[0]-60,wrist_int[1]-70),cv2.FONT_HERSHEY_SIMPLEX,.7,(0,255,0),2)
			elif gesture=='erasing':
				thumb_tip=self.get_landmark_coords(hand_landmarks,self.mp_hands.HandLandmark.THUMB_TIP);middle_tip=self.get_landmark_coords(hand_landmarks,self.mp_hands.HandLandmark.MIDDLE_FINGER_TIP)
				if thumb_tip and middle_tip:thumb_int=int(thumb_tip[0]),int(thumb_tip[1]);middle_int=int(middle_tip[0]),int(middle_tip[1]);cv2.circle(frame,thumb_int,10,(255,int(100*confidence),100),-1);cv2.circle(frame,middle_int,10,(255,int(100*confidence),100),-1);cv2.line(frame,thumb_int,middle_int,(255,int(100*confidence),100),3)
				if position:pos_int=int(position[0]),int(position[1]);cv2.circle(frame,pos_int,self.eraser_thickness//2,(255,255,255),2);cv2.circle(frame,pos_int,6,(255,0,0),-1)
				if wrist:cv2.putText(frame,'ACTIVE ERASING',(wrist_int[0]-60,wrist_int[1]-70),cv2.FONT_HERSHEY_SIMPLEX,.7,(0,0,255),2)
			elif gesture=='fill':
				if position:pos_int=int(position[0]),int(position[1]);cv2.circle(frame,pos_int,12,(255,0,255),3);cv2.circle(frame,pos_int,8,(255,255,255),-1)
				if wrist:cv2.putText(frame,'FILL TOOL',(wrist_int[0]-40,wrist_int[1]-70),cv2.FONT_HERSHEY_SIMPLEX,.7,(255,0,255),2)
	def run(self):
		left_gesture='none';right_gesture='none';show_interface=True
		try:
			while True:
				ret,self.frame=self.cap.read()
				if not ret:break
				self.frame=cv2.flip(self.frame,1)
				if self.canvas is None:self.canvas=np.zeros_like(self.frame)
				self.left_hand['active']=False;self.right_hand['active']=False;self.update_performance_metrics();self.check_hand_timeouts();rgb_frame=cv2.cvtColor(self.frame,cv2.COLOR_BGR2RGB);rgb_frame.flags.writeable=False;results=self.hands.process(rgb_frame);left_gesture='none';right_gesture='none'
				if results.multi_hand_landmarks:
					for(hand_index,hand_landmarks)in enumerate(results.multi_hand_landmarks):
						try:
							if results.multi_handedness and hand_index<len(results.multi_handedness):handedness=results.multi_handedness[hand_index];hand_label=handedness.classification[0].label
							else:hand_label='Right'
							hand_type='left'if hand_label=='Left'else'right';self.mp_draw.draw_landmarks(self.frame,hand_landmarks,self.mp_hands.HAND_CONNECTIONS,self.mp_draw.DrawingSpec(color=(0,255,0),thickness=3,circle_radius=4),self.mp_draw.DrawingSpec(color=(255,0,0),thickness=3))
							if hand_type=='left':gesture,position=self.process_left_hand_tools(hand_landmarks);left_gesture=gesture
				...
				combined=cv2.addWeighted(self.frame,.8,self.canvas,1.,0);self.draw_interface(combined,left_gesture,right_gesture,show_interface);cv2.imshow('Advanced Hand Drawing',combined);key=cv2.waitKey(1)&255
				if key==ord('q'):break
				elif key==ord('c'):self.save_state();self.canvas=np.zeros_like(self.frame)
				elif key==ord('s'):timestamp=datetime.now().strftime('%Y%m%d_%H%M%S');filename=f"saved_drawings/drawing_{timestamp}.png";cv2.imwrite(filename,self.canvas)
				elif key==ord('z'):self.undo()
				elif key==ord('y'):self.redo()
				elif key==ord('+')or key==ord('='):self.current_brush_size=min(self.current_brush_size+1,len(self.brush_sizes)-1);self.brush_thickness=self.brush_sizes[self.current_brush_size]
				elif key==ord('-'):self.current_brush_size=max(self.current_brush_size-1,0);self.brush_thickness=self.brush_sizes[self.current_brush_size]
				elif key==ord(']'):self.current_eraser_size=min(self.current_eraser_size+1,len(self.eraser_sizes)-1);self.eraser_thickness=self.eraser_sizes[self.current_eraser_size]
				elif key==ord('['):self.current_eraser_size=max(self.current_eraser_size-1,0);self.eraser_thickness=self.eraser_sizes[self.current_eraser_size]
				elif key==ord('t'):self.current_tool=(self.current_tool+1)%len(self.drawing_tools)
				elif key==ord(' '):show_interface=not show_interface
				elif key==ord('n'):self.current_color_index=(self.current_color_index+1)%len(self.colors);self.color=self.colors[self.current_color_index]
		except Exception as e:print(f"Critical error in main loop: {e}");traceback.print_exc()
		finally:
			try:self.cap.release();cv2.destroyAllWindows()
			except:pass
if __name__=='__main__':
	try:drawer=HandDrawing();drawer.run()
